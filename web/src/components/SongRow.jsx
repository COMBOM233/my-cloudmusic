...
import { useStore, setState, playSong, addToPlaylist, navigate } from '../store.js'
  ...
  return (
    <div className={'song-row' + (isCurrent ? ' current' : '')} onClick={() => playSong(song, queue)}>
      ...
      <span className="song-actions" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="btn-icon" title={liked ? '取消红心' : '红心'} onClick={(e) => { e.stopPropagation(); toggleLike() }}>{liked ? '❤️' : '🤍'}</button>
